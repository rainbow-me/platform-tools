...
func RequestContextUnaryServerInterceptor() grpc.UnaryServerInterceptor {
	return func(
		...
		parser := NewRequestParser(true, true)
...
// ParseMetadata extracts RequestInfo from gRPC metadata and ensures
...
// it is added to the gRPC metadata for downstream propagation.
//
// This function:
// - Extracts request/correlation/trace IDs from metadata
// - Generates a request ID ONLY if none is present in headers
// - Adds generated request ID to gRPC metadata for propagation (only if generated)
...
//   - *common.RequestInfo: Populated request information with guaranteed request ID
func (p *Parser) ParseMetadata(ctx context.Context) (context.Context, *common.RequestInfo) {
...
func (p *Parser) addRequestIDToMetadata(
	...
	newMD.Set(common.HeaderXRequestID, requestID)
...
func (p *Parser) extractAuthentication(md metadata.MD, info *common.RequestInfo) {
	// Authorization header
	if authHeader := p.getFirstValue(md, common.HeaderAuthorization); authHeader != "" {
		info.HasAuth = true
...
func (p *Parser) extractAllHeaders(md metadata.MD, info *common.RequestInfo) {
	info.AllHeaders = make(map[string]string)

	for key, values := range md {
		if len(values) > 0 {
			// Mask sensitive headers
			if p.maskSensitive && p.isSensitiveHeader(key) {
